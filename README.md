...
The first-party TypeScript source is **61 files across 7 modules** with **no circular dependencies**:

| Module | Files | Responsibility |
|--------|-------|----------------|
| `src/` | 16 | Orchestrator core: message loop, queue, IPC, container runner, scheduler, DB |
| `src/channels/` | 1 | Channel adapters (WhatsApp via baileys) |
| `skills-engine/` | 22 | `/update` & `/customize` machinery: apply, update, 3-way merge, rebase, replay, backup, state |
| `setup/` | 12 | `/setup` CLI steps (environment, container, auth, groups, register, mounts, service, verify) |
| `scripts/` | 6 | Maintenance CLIs (migrations, CI matrix, core update, post-update) |
| `container/agent-runner/src/` | 2 | Container-side agent runner (separate npm package; runs the Claude Agent SDK) |
| root | 2 | Vitest config |
...
The dependency and coverage reports are generated by [`tools/create-dependency-graph`](tools/create-dependency-graph/), which scans all first-party source (excluding `tools/` and non-source dirs); regenerate them with:
...
The suite is **476 tests across 44 files** covering the orchestrator, data layer, container/host runtimes, IPC authorization, mount security, scheduler, the skills engine (apply/update/merge/rebase/replay), and cross-platform setup. The dependency tool reports **60.7% file-level coverage** across all first-party source (37 of 61 files); the orchestrator core (`src/`) is the most thoroughly covered, while most untested files are `setup/` CLI steps, `scripts/`, and the `container/agent-runner/` package, which are exercised through the `/setup` and skill workflows rather than unit tests. See [TEST_COVERAGE.md](docs/architecture/TEST_COVERAGE.md) for the per-file detail.
