...
9. [First-Party Subsystems Beyond the Core](#first-party-subsystems-beyond-the-core)
10. [Testing](#testing)
...
Numbers below come from the autogenerated dependency report produced by
`tools/create-dependency-graph` on 2026-06-01. The dependency tool now scans the
whole first-party TypeScript surface — all 61 files across 7 modules — not just
`src/`. The orchestrator-core subset is broken out in
[Component Architecture](#component-architecture); the support subsystems are
described in [First-Party Subsystems Beyond the Core](#first-party-subsystems-beyond-the-core).

| Metric | Value |
|--------|-------|
| Source files (first-party TS) | 61 TypeScript files |
| Lines of code | 12,104 |
| Total exports | 240 |
| Re-exports (barrel) | 60 (`skills-engine/index.ts`) |
...
|--------|-------|----------|
| `src` | 16 | Orchestrator core: `index`, `config`, `env`, `logger`, `types`, `db`, `router`, `group-folder`, `group-queue`, `container-runner`, `container-runtime`, `mount-security`, `ipc`, `task-scheduler`, `fs-sync`, `whatsapp-auth` |
| `src/channels` | 1 | `whatsapp.ts` — `WhatsAppChannel` (baileys) |
| `skills-engine` | 22 | `/update` + `/customize` engine: `apply`, `update`, `merge`, `rebase`, `replay`, `backup`, `state`, `lock`, `path-remap`, `resolution-cache`, `git-utils`, `migrate`, `uninstall`, `manifest`, `customize`, `structured`, `file-ops`, `fs-utils`, `init`, `constants`, `types`, `index` (barrel) |
| `setup` | 12 | `/setup --step` CLI: `index`, `environment`, `container`, `whatsapp-auth`, `groups`, `register`, `mounts`, `service`, `verify`, `platform`, `status`, `node-script` |
| `scripts` | 6 | Maintenance CLIs: `apply-skill`, `update-core`, `post-update`, `run-migrations`, `generate-ci-matrix`, `run-ci-tests` |
| `container/agent-runner/src` | 2 | Separate npm package (`nanoclaw-agent-runner`): `index` (in-container Claude Agent SDK runner) + `ipc-mcp-stdio` (spawned MCP server) |
| `root` | 2 | `vitest.config.ts`, `vitest.skills.config.ts` |

The dependency graph is fully acyclic (0 circular dependencies, runtime or
type-only). Within the orchestrator core, `src/index.ts` is the main entry point
(imports from ~14 files, exports two test/helper functions); `src/logger.ts` and
`src/types.ts` are leaf dependencies imported almost everywhere.
`skills-engine/index.ts` is a barrel that re-exports 60 symbols from the other
21 engine files. See [DEPENDENCY_GRAPH.md](./DEPENDENCY_GRAPH.md) for the full
matrix.
...
|-------------|------|-------------|
| `src/index.ts` | main | Orchestrator process (message loop, scheduler, IPC watcher) |
| `src/whatsapp-auth.ts` | main | Standalone WhatsApp auth (npm `auth`) |
| `setup/index.ts` | CLI | `/setup` step runner (`--step <name>`) |
| `scripts/*.ts` | CLI | Maintenance commands (apply-skill, update-core, run-migrations, …) |
| `skills-engine/index.ts` | lib | Barrel API for the `/update` + `/customize` engine |
| `container/agent-runner/src/index.ts` | container | In-container Claude Agent SDK runner |
| `container/agent-runner/src/ipc-mcp-stdio.ts` | container | Spawned MCP server exposing the agent's IPC tools |
| `vitest.config.ts` / `vitest.skills.config.ts` | config | Test runner configs (core vs skills-engine suites) |
...
One Node.js orchestrator process whose core is ~16 source files, no
microservices, no message queues, no abstraction layers. The core is meant to be
read and understood in a sitting; the support subsystems (`skills-engine/`,
`setup/`, `scripts/`) exist mainly to make a *fork* updatable and installable.
Customization is expected to happen *in* this codebase, so it stays small on
purpose.
...
## First-Party Subsystems Beyond the Core

The dependency graph now covers all seven first-party modules. The
[Component Architecture](#component-architecture) section above describes the
`src/` orchestrator core and the `src/channels/` adapter. The remaining modules
are first-party support subsystems — part of the running/maintained project but
distinct from the message-loop core:

### `skills-engine/` — the `/update` + `/customize` engine (22 files)

The machinery behind the `/update` and `/customize` skills: applying, updating,
merging, rebasing, replaying, and migrating upstream NanoClaw changes onto a
user's customized fork. Key files: `apply.ts`, `update.ts`, `merge.ts`,
`rebase.ts`, `replay.ts`, `migrate.ts`, `uninstall.ts`, `backup.ts`, `lock.ts`,
`path-remap.ts`, `resolution-cache.ts`, `git-utils.ts`, `manifest.ts`,
`customize.ts`, `structured.ts`, `file-ops.ts`, `state.ts`, `init.ts`,
`constants.ts`, `types.ts`, and an `index.ts` barrel that re-exports 60 symbols.
It keeps a fork updatable despite local code changes — the linchpin of the
"customization = code" principle. It uses git plumbing (three-way `merge-file`,
`rerere` reuse, `diff --no-index`) to merge upstream onto a customized base.

### `setup/` — the `/setup` step runner (12 files)
...
`register`, `mounts`, `service`, `verify` (plus `platform`, `status`, and
`node-script` helpers, and `setup/index.ts` itself). It is what the `/setup`
skill invokes to install dependencies, authenticate WhatsApp, configure mounts,
register the main channel, and install the platform service. The setup steps
reuse core modules (`src/config.ts`, `src/db.ts`, `src/logger.ts`,
`src/group-folder.ts`) rather than duplicating them.

### `scripts/` — maintenance CLIs (6 files)

Thin command-line wrappers around the engines: `apply-skill.ts` (apply a skill),
`update-core.ts` (preview/apply an upstream update), `post-update.ts` (clear the
post-update backup), `run-migrations.ts` (run versioned migrations under tsx),
`generate-ci-matrix.ts` (compute the skill-overlap CI matrix), and
`run-ci-tests.ts` (drive the matrix in CI). Most import from `skills-engine/`.

### `container/agent-runner/` — the in-container agent (separate npm package, 2 files)

A standalone npm package (`nanoclaw-agent-runner`, with its own
`package.json`/`tsconfig.json` and dependencies) that runs the Claude Agent SDK
inside each container — or directly on the host in host mode. It is built
separately (`cd container/agent-runner && npm install && npm run build`) into
`dist/`. `src/index.ts` reads the `ContainerInput` from stdin, runs the SDK
`query()` loop (with agent-teams/swarm support), emits results between
`---NANOCLAW_OUTPUT_START/END---` markers, and consumes follow-up messages from
`/workspace/ipc/input/`. `src/ipc-mcp-stdio.ts` is the in-container MCP server
exposing the agent's tools (`send_message`, `schedule_task`, `list_tasks`,
`pause_task`, `resume_task`, `cancel_task`, `register_group`), which become the
request files the host's IPC watcher consumes. The `container/skills/` directory
(e.g. `agent-browser`) is synced into each group's `.claude/skills/`.

---

## Testing

Tests run under Vitest, split into two suites by config: `vitest.config.ts`
(orchestrator core + setup + scripts) and `vitest.skills.config.ts`
(`skills-engine/`). The suite covers **476 tests across 44 test files**, giving
roughly **60.7% file-level coverage** (37 of the 61 first-party source files
have at least one direct test). Coverage is densest in the security- and
correctness-critical areas — IPC authorization, mount security, group-folder
path safety, the group queue, and the skills-engine merge/rebase machinery. See
[TEST_COVERAGE.md](./TEST_COVERAGE.md) for the autogenerated per-file breakdown.
