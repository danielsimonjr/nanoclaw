...
and IPC namespace. The orchestrator core is intentionally small — 16 source
files in `src/` (plus the one-file `src/channels/` adapter) — so you can read
it, understand it, and customize it (via Claude Code) to fit your exact needs.
Around that core sit a few first-party support subsystems (`skills-engine/`,
`setup/`, `scripts/`, and the separate `container/agent-runner/` package),
bringing the whole TypeScript codebase to 61 files across 7 modules.
...
| Cross-platform | macOS, Linux, and native Windows 10/11 (orchestrator native; agents in Linux containers via Docker WSL2, or directly on the host with `CONTAINER_RUNTIME=host`) |
...
The first-party TypeScript source is 61 files across 7 modules (16 in `src/`,
1 in `src/channels/`, 22 in `skills-engine/`, 12 in `setup/`, 6 in `scripts/`,
2 in `container/agent-runner/src/`, plus 2 root vitest configs).

```
nanoclaw/
├── src/                       # Orchestrator core (16 TS files)
...
│   ├── whatsapp-auth.ts       #   Standalone WhatsApp auth script (npm `auth`)
│   └── channels/whatsapp.ts   #   WhatsAppChannel (baileys) — 1-file channels module
├── skills-engine/             # /update + /customize engine (22 TS files): apply, update,
│                              #   merge, rebase, replay, backup, state, lock, path-remap,
│                              #   resolution-cache, git-utils, migrate, uninstall, …
├── setup/                     # /setup --step CLI (12 TS files): environment, container,
│                              #   whatsapp-auth, groups, register, mounts, service, verify,
│                              #   plus platform/status/node-script helpers
├── scripts/                   # Maintenance CLIs (6 TS files): apply-skill, update-core,
│                              #   post-update, run-migrations, generate-ci-matrix, run-ci-tests
├── container/                 # Agent sandbox
│   ├── Dockerfile / build.sh  #   Agent image build
│   ├── agent-runner/          #   Separate npm package (nanoclaw-agent-runner): in-container
│   │                          #     Claude Agent SDK runner (index.ts) + IPC MCP stdio server
...
[DEPENDENCY_GRAPH.md](./DEPENDENCY_GRAPH.md). The dependency graph now covers all
seven first-party modules (`src/`, `src/channels/`, `skills-engine/`, `setup/`,
`scripts/`, `container/agent-runner/src/`, and the root vitest configs), with
**0 circular dependencies** (runtime or type-only).

## Key design principles

1. **Small enough to understand** — one orchestrator process whose core is ~16
   source files, no microservices or abstraction layers.
