...
describe('register_group authorization', () => {
  ...
  it('main can register a brand-new jid + folder', async () => {
    await processTaskIpc(
      {
        type: 'register_group',
        jid: 'fresh@g.us',
    ...
    expect(groups['fresh@g.us']?.folder).toBe('fresh-group');
  });

  it('rejects re-binding an existing folder to a different jid (hijack guard)', async () => {
    // 'other-group' folder is already owned by other@g.us.
    await processTaskIpc(
      {
        type: 'register_group',
        jid: 'attacker@g.us',
  ...
  it('rejects re-pointing an existing jid to a different folder', async () => {
    await processTaskIpc(
      {
        type: 'register_group',
        jid: 'other@g.us',
        name: 'Other',
        folder: 'some-new-folder',
        trigger: '@Andy',
      },
      'main',
      true,
    ...
    // other@g.us still maps to its original folder.
    expect(groups['other@g.us'].folder).toBe('other-group');
  });
});
...
describe('schedule_task context_mode', () => {
  ...
  it('defaults invalid context_mode to group (matches the schedule_task tool default)', async () => {
    await processTaskIpc(
      {
        type: 'schedule_task',
        prompt: 'bad context',
